...
proc mountFile {f} {
    ...
    try {
        ...
        # puts stderr [list $f $msg]
    } 
    return ""
}

proc unmountFile {f mnt} {
    if {$mnt ne ""} {
        vfs::sevenzip::Unmount $f $mnt
    }
}

## Code to delete a node of the tree and unmount archives
proc deleteTree {tree node} {
    foreach id [$tree children $node] {
        deleteTree $tree $id 
        unmountFile [$tree set $id fullpath] [$tree set $id mnt]
        $tree delete $id
    }
}
...
proc populateTree {tree node} {
    ...
    foreach f [lsort -dictionary [glob -nocomplain -dir $path *]] {
        ...
        if {$type eq "directory"} {
            ...
            if {[set mnt [mountFile $f]] ne ""} {
                $tree insert $id 0 -text dummy ;# a dummy
                $tree item $id -text [file tail $f]
                $tree set $id mnt $mnt
                $tree set $id type "directory"
            }
        }
    }
...
ttk::style configure Treeview \
        -rowheight [font metrics [ttk::style lookup Treeview -font] -linespace]
ttk::treeview $w.tree -columns {fullpath type size mnt} -displaycolumns {size} \
